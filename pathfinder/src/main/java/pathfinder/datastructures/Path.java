...
/**
 * This represents an immutable path between two cartesian coordinate elements, particularly
 ...
 * @param <E> the type of elements used by path
 */
public class Path<E> implements Iterable<Path<E>.Segment<E>> {

    // AF(this) =
    //      first object in the path => start
    //      each "step" along the path between objects (of type E) => elements of list path, where
    //          path.get(0) is the first step from the start object to an intermediate object, and
    //          path.get(path.size() - 1) is the final step from an intermediate generic object to the end
    //      total cost along the path => cost
    //      the destination generic object in this path, opposite the start generic object => getEnd()
    ...
    /**
     * The generic object at the beginning of this path.
     */
    private E start;
     ...
     */
    private List<Path<E>.Segment<E>> path;

    /**
     * Creates a new, empty path containing a start generic object. Essentially this represents a path
     * from the start generic object to itself with a total cost of "0".
     *
    ...
    /**
     * Appends a new single segment to the end of this path, originating at the current last generic object
     ...
     * segmentCost}.
     *
     * @param newEnd      The generic object being added at the end of the segment being appended to this path
    ...
    /**
     * @return The generic object at the end of this path, which may be the start generic object if this path
     ...
     * generic object and ending at the end generic object. In the case that this path represents a path between
     * the start generic object and itself, this iterator contains no elements. This iterator does not
     ...
     */
    @Override
    public Iterator<Path<E>.Segment<E>> iterator() {
        // Create a wrapping iterator to guarantee exceptional behavior on Iterator#remove.
        return new Iterator<>() {

            private Iterator<Path<E>.Segment<E>> backingIterator = path.iterator();
    ...
    private void checkRep() {
        ...
        assert start != null;
        assert path != null;
        for(Path<E>.Segment<E> segment : path) {
            assert segment != null;
        }
    }
     ...
     * both paths are empty, they are only equal if their starting generic object is equal.
     *
    ...
    /**
     * Segment represents a single segment as part of a longer, more complex path between generic objects.
     * Segments are immutable parts of a larger path that cannot be instantiated directly, and
     * are created as part of larger paths by calling Path#extend(element, double).
     */
    public class Segment<V> {
         ...
         *
         * @param start The starting generic object of this segment.
         * @param end   The ending generic object of this segment.
         ...
         */
        public V getStart() {
            // Note: Since generic objects are immutable, this isn't rep exposure.
            return this.start;
        }

        /**
         * @return The ending generic object of this segment.
         */
        public V getEnd() {
            return this.end;
        }
