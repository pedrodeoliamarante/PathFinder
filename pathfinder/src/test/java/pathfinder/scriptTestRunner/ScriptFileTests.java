...
public class ScriptFileTests {
    ...
    public static List<Object[]> getTestFiles() {
        ...
        try {
            ...
            if (!baseDir.exists()) {
                System.err.println("There are no script tests to run.");
                System.err.println("Expected in a directory at: " + baseDir);
                return filesToTest; // Empty
            }
            ...
            if (contents == null) {
                throw new RuntimeException("Error accessing files in: " + baseDir);
            }
            for (File f : contents) {
    ...
    private static String fileContents(File f) throws IOException {
        ...
        if (!f.isFile()) {
            throw new IllegalArgumentException("File isn't a standard file: " + f);
        }
